<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Create New Question for Exam:') }} "{{ $exam->title }}"
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700" x-data="questionForm()">
                    <form method="POST" action="{{ route('admin.exams.questions.store', $exam) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="question_text" id="question_text" rows="4" required class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200">{{ old('question_text') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                            <div>
                                ...
                                <select name="type" id="type" x-model="questionType" required class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200">
                            ...
                            <div>
                                ...
                                <input type="number" name="points" id="points" value="{{ old('points', 1) }}" required min="1" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200">
                        ...
                        <div x-show="questionType === 'multiple_choice'" class="mt-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Add at least 2 options and select the correct one.</p>

                            <div id="options-container" class="space-y-3">
                                <template x-for="(option, index) in options" :key="index">
                                    <div class="flex items-center space-x-3 p-2 rounded-md bg-gray-50 dark:bg-gray-700/50">
                                        <input type="radio" name="correct_option" :value="index" class="form-radio h-5 w-5 text-indigo-600 focus:ring-indigo-500" required>
                                        <input type="text" :name="'options[' + index + '][text]'" x-model="option.text" placeholder="Option text" class="flex-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-900 dark:text-gray-200" required>
                        ...
                        <div class="mt-8 flex justify-end">
                            ...
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">Create Question</button>
                        </div>
                    </form>
                </div>
            </div>
